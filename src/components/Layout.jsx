    ...
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row max-w-[800px] w-full mx-auto shadow-sm">
      ...
      <aside className="hidden md:flex flex-col w-56 bg-white border-r border-gray-200 p-6">
        ...
        {showNav ? (
          ...
          <nav className="flex flex-col gap-4">
            <Link to="/about" className={`flex items-center gap-3 p-3 rounded-xl transition-colors ${location.pathname === '/about' ? 'bg-green-50 text-green-700 font-semibold' : 'text-gray-600 hover:bg-gray-50'}`}>
              <BookOpen size={20} />
              <span>서비스 소개</span>
            </Link>
          </nav>
      ...
      {/* Mobile Bottom Navigation — 로그인/게스트 접속 시에만 표시 */}
      {showNav ? (
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex justify-center items-center h-16 px-4 z-50">
